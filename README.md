...
Spring context에 선언이 아닌 프로그램으로 bean을 추가하는 방법이다. 프로그램적으로 특정 조건에 맞는 인스턴스만 bean으로 만들 수 있다.
- `Beans`에 추가
    ```kotlin
    @Configuration
...
- `SpringContext`에 추가
    ...
    ```
- registerBean()의 3번째 파라미터는 Java에서 말하는 functional interface 구현체로 bean 객체를 공급하는 역할이다. Supplier { } 안에는 인스턴스를 반환하는 람다식을 넣는다. 

## 2.2. Bean 주입시키기
- 객체들 간의 의존관계가 필요할 경우 context에 있는 bean을 가져와 사용할 수 있다.
- UML 기준으로 dependency, association, aggregation, composition 관계를 맺을 때 사용할 수 있다.
- 방법들
...
- `Beans`에 수정 및 추가
  ...
  ```
  - @Configuration에 CarOnwer와 Car bean을 등록한다. 이 때 car2() bean 함수를 직접 호출해서 car 멤버 속성에 주입하였다.
  - CarOwner와 Car 객체 간에 aggregation (has-a) 관계가 만들어진다.
  - **만일 context 안에 이미 car2 bean이 존재한다면, car2() 함수가 실제로는 호출되지 않고 context에서 bean을 가져와서 주입시켜 준다.**
- `SpringContext`에 추가
  ```kotlin
  fun composeBean() {
...
-`SpringContext` 수정
  ...
  ```
- [ ] 문제: 두번째 getBean() 호출하면 CarOwner("carOwner1", car1())에 의하여 @Bean car1()이 또 다시 호출될까? 
...
- `Beans`에 수정
  ...
  ```
  - context에 CarOwner 타입의 bean이 있으면 carOwner 값으로 주입시켜준다. 따라서 context bean이 @Bean으로 등록되었는지 @Component로 주입되었는지 경로는 상관없다.
  - Spring이라는 IoC container가 DI(Dependency Injection)을 수행하는 장면이다.
- `SpringContext`에 추가
  ```kotlin
  fun composeBean() {
      // ...
